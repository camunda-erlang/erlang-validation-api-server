...
-export([create_process_id_cache/0]).
-export([add_process_id_cache/2]).
-export([del_process_id_cache/1]).
-export([get_process_id_cache/1]).
...
%----------------------------------------------------------------------------------------------
create_process_id_cache() ->
	ets_new(process_id_cache, [public, set, named_table, {read_concurrency, true}]).
add_process_id_cache(Key, Data) ->
	ets_insert(process_id_cache, Key, Data).
del_process_id_cache(Key) ->
	ets_delete(process_id_cache, Key).
get_process_id_cache(Key) ->
	ets_lookup(process_id_cache, Key).		

% ets_insert(Tab, ValueList) when is_list(ValueList) ->	
	% ets:insert(Tab, ValueList);
...
ets_lookup(Tab, Key) ->
	case catch ets:lookup(Tab, Key) of
		{'EXIT', _Reason} ->
			io:fwrite("ets_lookup Tab:~p Key:~p~n", [Tab, Key]),
			not_found;
		[{_, Value}] -> 
			Value;
		_ -> 
...
ets_new(Tab, Opts)	->
	case catch ets:info(Tab) of
		{'EXIT', Reason} ->
			io:fwrite("ets_new EXIT with reason :~p~n", [Reason]),
			(catch ets:new(Tab, Opts));
		undefined ->
			(catch ets:new(Tab, Opts));
		Info ->
			io:fwrite("ets_new ets table exists Info :~p~n", [Info])
	end.